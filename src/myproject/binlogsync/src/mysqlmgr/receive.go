...
func (mgr *MysqlMgr) handlerdbinsert(buf []byte) int {
	...
	mgr.Logger.Infof("handlerdbinfo len: %+v", q.Data)
...
func (mgr *MysqlMgr) InsertMultiStreamInfos(data DataDetail, tablename string) int {	
	insertsql := "INSERT INTO " + tablename + " (task_id,file_name,file_type,file_size,domain,status, " + 
